...
export default async function HomePage() {
  const language=await getRequestLanguage(); const t=getUi(language).home; const services=getServices(language); const faqs=getFrequentlyAskedQuestions(language); const bookingUrl=getWhatsAppUrl(getGeneralBookingMessage(language));
  const homeServices=[
    {...services[0],label:t.labels[0]},
    {...services[1],label:t.labels[1]},
    {slug:"helhetslosning",title:language==="sv"?"Packning & städning":"Packing & cleaning",short:language==="sv"?"Vi tar hand om detaljerna före och efter.":"We handle the details before and after.",description:language==="sv"?"Komplettera flytten med professionell packhjälp, material och noggrann flyttstädning.":"Complete your move with professional packing, materials and thorough move-out cleaning.",image:"/images/packing.avif",message:language==="sv"?"Hej Flyttiva! Jag vill gärna ha hjälp med packning och flyttstädning.":"Hello Flyttiva! I would like help with packing and move-out cleaning.",label:t.labels[2]},
  ];
  return <>
    <section className="lux-hero"><div className="lux-hero__grid shell"><div className="lux-hero__copy"><h1>{t.heroTitle}</h1><p>{t.heroText}</p><BookingDialog language={language} /></div><div className="lux-hero__visual"><Image src="/images/hero.avif" alt={t.heroAlt} fill priority sizes="(max-width: 800px) 100vw, 48vw" /></div></div></section>
    ...
    <section className="faq-section section" id="faq"><div className="faq-layout shell"><div className="faq-intro"><span className="kicker">{t.faqKicker}</span><h2>{t.faqTitle}</h2><p>{t.faqText}</p><a className="quiet-link" href={bookingUrl} target="_blank" rel="noreferrer">{t.askQuestion} <ArrowUpRight /></a></div><div className="faq-list">{faqs.map((item,index)=><details key={item.question} open={index===0}><summary><span>0{index+1}</span>{item.question}<i>+</i></summary><p>{item.answer}</p></details>)}</div></div></section>
